...
class OptionParser < UI::CLI::OptionParser
    ...
    def scope
        ...
        on( '--scope-exclude-file-extensions EXTENSION,EXTENSION2,..',
            ...
            "(Default: #{options.scope.exclude_file_extensions.to_a.join( ',' )})"
        ) do |extensions|
    ...
    def http
        ...
        on( '--http-request-header NAME=VALUE',
            ...
            if RKN.edition == :community
                print_error "--http-request-header option not supported by #{RKN.edition} edition."
                exit 1
            end
        ...
        on( '--http-cookie-jar COOKIE_JAR_FILE',
            ...
            if RKN.edition == :community
                print_error "--http-cookie-jar option not supported by #{RKN.edition} edition."
                exit 1
            end
        ...
        on( '--http-cookie-string COOKIE',
            ...
            if RKN.edition == :community
                print_error "--http-cookie-string option not supported by #{RKN.edition} edition."
                exit 1
            end
        ...
        on( '--http-authentication-username USERNAME',
            ...
            if RKN.edition == :community
                print_error "--http-authentication-username option not supported by #{RKN.edition} edition."
                exit 1
            end
        ...
        on( '--http-authentication-password PASSWORD',
            ...
            if RKN.edition == :community
                print_error "--http-authentication-password option not supported by #{RKN.edition} edition."
                exit 1
            end
        ...
        on( "--http-ssl-certificate-type #{SCNR::Engine::OptionGroups::HTTP::SSL_CERTIFICATE_TYPES.join(',')}",
            SCNR::Engine::OptionGroups::HTTP::SSL_CERTIFICATE_TYPES,
        ...
        on( "--http-ssl-key-type #{SCNR::Engine::OptionGroups::HTTP::SSL_KEY_TYPES.join(',')}",
            SCNR::Engine::OptionGroups::HTTP::SSL_KEY_TYPES,
            'SSL key type.'
        ) do |type|
        ...
        on( "--http-ssl-version #{SCNR::Engine::OptionGroups::HTTP::SSL_VERSIONS.join(',')}",
            SCNR::Engine::OptionGroups::HTTP::SSL_VERSIONS,
            'SSL version to use.'
        ) do |type|
    ...
    rescue SCNR::Engine::Options::Error::InvalidURL => e
        print_bad e
        exit
    end
