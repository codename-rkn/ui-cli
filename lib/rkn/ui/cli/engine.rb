...
class Engine
    ...
    def initialize
        ...
        @application.options = options.to_h

        @scan = @application.scnr_api.scan
    ...
    def print_entries( unmute = false )
        super( @application.entries.values, unmute )
    end
    ...
    def generate_reports
        ...
        if !File.directory?( options.report.path )
            ...
            print_line
            print_info "Report saved at: #{options.report.path} [#{filesize}MB]"
        end
    end
